...
def parse_name_samples(
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        lines = f.readlines()
        print(f"[Initialization] Read {len(lines)} lines in sample file")
        for line_num, line in enumerate(lines, 1):
            ...
            target_token_position = valid_context_len + len(tokenizer.tokenize(target_name)) - 1
    ...
    print(f"\n[Parsing Completed] Obtained {len(samples)} valid samples (MRR≥{min_mrr})")
    return samples
...
def main():
    ...
    name_samples = parse_name_samples(
        file_path=config["name_sample_path"],
        tokenizer=tokenizer,
    ...
    all_lrp_results = []
    total_samples = len(name_samples)
    with tqdm(total=total_samples, desc="LRP calculation progress") as pbar:
        for batch_start in range(0, total_samples, config["batch_size"]):
            batch_samples = name_samples[batch_start:batch_start + config["batch_size"]]
    ...
    print(f"Number of valid results: {len(all_lrp_results)}")

    print("\n=== Full layer validity verification ===")
    ...
    for layer_idx in sorted(layer_valid_count.keys()):
        valid_rate = layer_valid_count[layer_idx] / len(all_lrp_results) * 100
        print(f"Layer {layer_idx}: Valid sample rate={valid_rate:.1f}% ({layer_valid_count[layer_idx]}/{len(all_lrp_results)})")

    print("\n=== LRP positive and negative sign distribution statistics ===")
    ...
    for layer_idx in sorted(layer_sign_stats.keys()):
        ...
        print(f"Layer {layer_idx}: Positive contribution neuron ratio={pos_rate:.1f}% | Negative contribution neuron ratio={neg_rate:.1f}% | Zero contribution={zero_rate:.1f}%")
