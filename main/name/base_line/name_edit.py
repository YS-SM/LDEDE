...
def load_gpt2_privacy_neurons(neuron_path: str) -> Dict[int, List[int]]:
    ...
    if not isinstance(privacy_neurons, dict) or len(privacy_neurons) == 0:
        raise ValueError("Compatible dictionary format error (must be a dictionary of layer→neuron list)")
...
class GPT2NamePrivacyEvaluator:
    ...
    def analyze_mrr_by_interval(self) -> None:
        ...
        intervals = [
            ("High memory", 0.8, 0.9),
            ("Medium memory", 0.6, 0.7),
            ("Low memory", 0.4, 0.5)
        ]

        print(f"\n=== MRR Interval Stratified Analysis ===")
        print(f"Interval Type   MRR Range    Sample Count    Avg MRR (Before)   Avg MRR (After)   Reduction Rate    ")
        print("--------------------------------------------------------------------------")
