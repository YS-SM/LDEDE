...
def select_privacy_neurons(
    ...
    print(f"\n=== Privacy neuron selection configuration (positive LRP neurons only) ===")
    ...
    print(f"Sorting rule: Sort by vote ratio descending (for ties, average LRP → vote count → layer index)")
    print(f"Final selection: Top-{top_z} positive LRP neurons (promote privacy leakage)")
    ...
    if len(qualified_neurons) < top_z:
        print(f"[Warning] The number of qualified positive LRP neurons ({len(qualified_neurons)}) is less than {top_z}, all will be selected")
    ...
    for layer_idx in sorted(compatibility_dict.keys()):
        ...
        print(
            f"  Layer {layer_idx}: {neuron_count} neurons | Average vote ratio {avg_vote_freq:.2%} | Average LRP {avg_lrp:.4f} | Top 5 indices: {top5_neurons}")

    print(f"\nTop 10 core positive LRP neurons details (promote privacy leakage):")
    for i, (layer_idx, neuron_idx, votes, vote_freq, avg_lrp) in enumerate(top_neurons[:10], 1):
        print(
            f"  Rank {i}: Layer {layer_idx} | Neuron {neuron_idx} | Votes {votes} | Vote ratio {vote_freq:.2%} | LRP value {avg_lrp:.4f}")
    print("=" * 80 + "\n")
...
if __name__ == "__main__":
    ...
    config = {
        "lrp_npy_path": r"D:\expt\lrp_spin\main\name\base_line\gpt2_name_integrated_grad_20steps_results_grad_20steps.npy",
        "save_neuron_path": r"D:\expt\lrp_spin\main\name\base_line\gpt2_name_neurons_voted_top5000_grad_20steps.npy",
        "top_z": 5000,
        "global_top_ratio": 0.5,
        "vote_ratio_threshold": 0.4,
        "layer_range": (0, 11),
        "num_processes": 20,
