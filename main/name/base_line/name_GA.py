...
def precompute_ffn_input(
    ...
    for i in range(input_ids.shape[0]):
        ...
        target_text = sample["target_name"]
...
def parse_name_samples(
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        lines = f.readlines()
        print(f"[Initialization] Read {len(lines)} lines from sample file")
    ...
    print(f"\n[Parsing Complete] Obtained {len(samples)} valid samples (MRR≥{min_mrr})")
    return samples
...
def main():
    ...
    print("\n=== Step 1: Parse High-Memorization Name Samples ===")
