...
def register_gpt2_privacy_hooks(
    ...
    if not hooks:
        raise RuntimeError("Failed to register any suppression hooks, please check the validity of layer indices")
...
def parse_high_memory_email_samples(sample_file_path: str, tokenizer: GPT2Tokenizer) -> List[Dict]:
    ...
    print(f"Sample parsing completed: {len(sample_details)} valid samples")
...
def print_stratification_report(strat_stats: Dict[str, Dict]):
    ...
    print("=" * 80)
    print(
        f"{'Range Name':<18} {'PPL Range':<10} {'Sample Count':<12} {'Original Avg PPL':<16} {'Suppressed Avg PPL':<18} {'Avg Increase Ratio':<18} {'Effect Evaluation'}")
    print("-" * 80)
    ...
    if "Other range" in strat_stats and strat_stats["Other range"]["sample_count"] > 0:
        ...
        print(
            f"{'Other range':<18} {'1.0-9.0 outside':<10} {other_stats['sample_count']:<12} "
    ...
    if total_main_samples > 0:
        ...
        print("-" * 80)
        print(
            f"{'Main ranges summary':<18} {'—':<10} {total_main_samples:<12} {total_original_ppl:<16.2f} "
            f"{total_suppressed_ppl:<18.2f} {total_increase_ratio:<18.1f}% {'—'}"
        )
    else:
        print("-" * 80)
        print(f"{'Main ranges summary':<18} {'—':<10} 0            -                -                  -                  —")
    ...
    if main_stats:
        ...
        print(f"   - Best suppression effect range: {best_label} (increase {best_strat['avg_increase_ratio']:.1f}%)")
        ...
        print(
            f"   - Most concentrated sample range: {largest_label} ({largest_strat['sample_count']} samples, {largest_strat['sample_count'] / total_main_samples * 100:.1f}% of main ranges)")
        if "High memory range" in main_stats:
            high_memory_stats = main_stats["High memory range"]
            print(
                f"   - High memory range (strongest memory) suppression effect: {'Excellent' if high_memory_stats['avg_increase_ratio'] > 20 else 'Good' if high_memory_stats['avg_increase_ratio'] > 10 else 'Insufficient'} (average increase {high_memory_stats['avg_increase_ratio']:.1f}%)")
    else:
        print(f"   - No valid samples in main ranges, cannot evaluate core suppression effect")

    if "Other range" in strat_stats and strat_stats["Other range"]["sample_count"] > 0:
        other_count = strat_stats["Other range"]["sample_count"]
        total_all_samples = total_main_samples + other_count
        print(f"   - Other range samples: {other_count} ({other_count / total_all_samples * 100:.1f}% of total samples), not included in core analysis")
...
def evaluate_privacy_suppression_effect(
    ...
    if len(sample_details) < 3:
        raise ValueError(f"Too few valid samples ({len(sample_details)}), evaluation requires ≥3 samples")

    print("\nStarting privacy suppression effect evaluation")
    evaluation_results = []
    ...
    print(
        f"Conclusion: {'Privacy suppression effective (memory significantly weakened)' if avg_increase_ratio > 15 else 'Effect moderate (memory slightly weakened)' if avg_increase_ratio > 5 else 'Privacy suppression ineffective'}")
    print(f"{'=' * 60}")
...
def main():
    config = {
        ...
        "neuron_path": r"D:\expt\lrp_spin\main\email\base_line\gpt2_emails_neurons_voted_top5000_grad_20steps.npy",
    ...
    try:
        ...
        if original_general_ppl != float('inf') and suppressed_general_ppl != float('inf'):
            ...
            print(
                f"   - Performance conclusion: {'No significant loss' if general_increase_ratio < 30 else 'Minor loss' if general_increase_ratio < 50 else 'Severe loss'}")

        print(f"\nPrivacy suppression evaluation:")
        print(f"   - Valid evaluation samples: {len(privacy_results)}")
        print(f"   - Original model average privacy PPL: {avg_original_privacy:.2f} (Lower PPL = stronger memory)")
        print(f"   - Suppressed model average privacy PPL: {avg_suppressed_privacy:.2f} (Higher PPL = weaker memory)")
        print(f"   - Average PPL increase ratio: {avg_increase_ratio:.1f}%")
        if len(privacy_results) >= 3:
            if avg_increase_ratio > 15:
                ...
                privacy_conclusion = "Moderate effect (sensitive information memory slightly weakened)"
            else:
                privacy_conclusion = "Ineffective (no significant change in sensitive information memory)"
            print(f"   - Privacy conclusion: {privacy_conclusion}")

        if strat_stats:
            print(f"\nStratification analysis core conclusions:")
            main_intervals = ["High memory range", "Medium memory range", "Low memory range"]
            main_stats = {k: v for k, v in strat_stats.items() if k in main_intervals and v["sample_count"] > 0}
            if main_stats:
                best_strat = max(main_stats.values(), key=lambda x: x["avg_increase_ratio"])
                ...
                print(
                    f"   - Main ranges sample distribution: {total_main_samples} samples ({total_main_samples / len(privacy_results) * 100:.1f}% of valid evaluation samples)")
