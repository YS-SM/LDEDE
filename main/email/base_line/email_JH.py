...
def select_privacy_neurons(
    ...
    print(f"Sorting rule: Descending by vote rate (for ties, descending by average LRP → vote count → layer index)")
    ...
    for layer_idx in sorted(compatibility_dict.keys()):
        ...
        print(
            f"  Layer {layer_idx}: {neuron_count} neurons | Average vote rate {avg_vote_freq:.2%} | Average LRP {avg_lrp:.4f} | Top 5 indices: {top5_neurons}")
    ...
    for i, (layer_idx, neuron_idx, votes, vote_freq, avg_lrp) in enumerate(top_neurons[:10], 1):
        print(
            f"  Rank {i}: Layer {layer_idx} | Neuron {neuron_idx} | Votes {votes} | Vote rate {vote_freq:.2%} | LRP value {avg_lrp:.4f}")
    print("=" * 80 + "\n")
...
if __name__ == "__main__":
    ...
    config = {
        "lrp_npy_path": r"D:\expt\lrp_spin\main\email\base_line\gpt2_email_integrated_grad_20steps_results.npy",
        "save_neuron_path": r"D:\expt\lrp_spin\main\email\base_line\gpt2_emails_neurons_voted_top5000_grad_20steps.npy",
        "top_z": 5000,
