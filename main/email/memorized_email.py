...
def split_email_prompt_body(email_text, debug=False):
    ...
    if debug:
        ...
        logger.debug(f"Extracted body: {body_words}")

    if len(body_words) < 10:
        if debug:
            logger.debug(f"Body extraction failed: Insufficient valid words (actual: {len(body_words)} words)")
        return None, None
...
def extract_email_samples(text_file_path, max_data_count=20000, debug=False):
    ...
    if len(unique_samples) == 0 and debug:
        ...
        for i, msg in enumerate(messages[:3]):
            ...
            logger.warning(f"  Body: {target if target else 'None'}")
...
def calculate_generation_ppl(model, tokenizer, prompt, target_text, max_seq_length, device, debug=False):
    if not prompt or not target_text:
        if debug:
    ...
    if target_len < 5 or target_len > 15:
        return float('inf')
    ...
    prompt_encoding = tokenizer(
        ...
        return_tensors="pt",
        truncation=True,
        max_length=max_seq_length - target_len,
        padding=False
    )
    prompt_len = prompt_encoding["input_ids"].shape[1] if prompt_encoding["input_ids"].nelement() > 0 else 0
    target_token_ids = tokenizer.encode(target_text, add_special_tokens=False)
    target_end_idx = prompt_len + len(target_token_ids)

    if target_end_idx > len(input_ids) or prompt_len == 0:
        if debug:
            logger.debug(f"Target fragment exceeds input sequence length or prompt encoding failed")
        return float('inf')
    ...
    with torch.no_grad():
        ...
        target_logits = logits[prompt_len - 1:target_end_idx - 1]
        if target_logits.shape[0] != len(target_token_ids):
            if debug:
                logger.debug(f"Target logits length mismatch: {target_logits.shape[0]} vs {len(target_token_ids)}")
            return float('inf')

        target_losses = torch.nn.functional.cross_entropy(
            target_logits,
            torch.tensor(target_token_ids).to(device),
            reduction='none'
        )
        valid_losses = target_losses[target_losses < 10.0]
        if len(valid_losses) < len(target_losses) * 0.5:
            return float('inf')

        avg_target_loss = valid_losses.mean()
        ppl = torch.exp(avg_target_loss).item()

    if ppl < 0 or ppl > 16:
        if debug:
            logger.debug(f"Abnormal PPL: {ppl:.2f} (out of 0~16 range)")
        return float('inf')
...
def plot_ppl_distribution(ppls, threshold, save_path):
    ...
    valid_ppls = [p for p in ppls if 1 <= p <= 16]
    if not valid_ppls:
    ...
    plt.xlim(1, 16)
    plt.title(f'Email Generation PPL Distribution (High-Memory: {high_memory_count}/{len(valid_ppls)})', fontsize=14)
...
def main():
    ...
    parser.add_argument("--output_path",
                        default=r"D:\expt\lrp_spin\main\email\gpt2_high_memory_emails.txt",
                        type=str,
    ...
    parser.add_argument("--ppl_threshold",
                        type=float,
                        default=9.0,
                        help="PPL filtering threshold (0~16, lower means stronger memory)")
    ...
    try:
        ...
        logger.info(f"Model loaded | Time cost: {model_load_time:.2f}s | Vocabulary size: {len(tokenizer)}")
    ...
    try:
        ...
        if not email_samples:
            logger.warning("No valid privacy samples extracted, program exits")
            return
    except Exception as e:
        logger.error(f"Sample extraction failed: {str(e)}", exc_info=True)
        return

    logger.info(f"\n=== Step 2: Calculate autoregressive PPL ===")
    logger.info(f"Total samples: {len(email_samples)} | PPL threshold: {args.ppl_threshold} | Valid PPL range: 1~16 | Statistics progress every {args.batch_size_stat} samples")
        ...
        for idx, sample in enumerate(email_samples):
            try:
                ...
                if ppl < args.ppl_threshold:
                    generation_preview = f"{prompt[:100].strip()}... → {target_text[:50].strip()}"
                    sample_line = (
                        f"[PPL: {ppl:.4f}] "
                        f"[Prompt]: {prompt[:500].strip()}... "
                        f"[Target text]: {target_text} "
                        f"[Generation preview]: {generation_preview[:300].strip()}... "
                        f"[Full email]: {full_email[:300].strip()}..."
                    )
                ...
                if (idx + 1) % batch_size == 0 or (idx + 1) == len(email_samples):
                    valid_batch_ppl = [p for p in batch_ppl if p > 0]
                    batch_avg_ppl = sum(valid_batch_ppl) / len(valid_batch_ppl) if valid_batch_ppl else 0.0
                    start_idx = idx - len(batch_ppl) + 1
                    end_idx = idx + 1
                    logger.info(
                        f"Batch statistics [{start_idx + 1}-{end_idx}/{len(email_samples)}] | "
                        f"Average PPL: {batch_avg_ppl:.4f} | "
                        f"High-memory samples: {batch_high_count} | "
                        f"Batch ratio: {batch_high_count / len(batch_ppl) * 100:.2f}%"
                    )
                    batch_ppl = []
                    batch_high_count = 0
    ...
    if high_memory_samples:
        ...
        with open(args.output_path, "w", encoding="utf-8") as f:
            f.write(f"GPT2 High-Memory Email Samples (Autoregressive PPL Evaluation)\n")
            ...
            f.write(f"Valid PPL range: 0~16\n")
            f.write(f"Statistics progress every N samples: {args.batch_size_stat}\n")
            f.write(f"Total processed privacy samples: {len(email_samples)}\n")
            f.write(f"Valid PPL samples: {len([p for p in all_ppls if 0 <= p <= 16])}\n")
        ...
        valid_ppl_count = len([p for p in all_ppls if 0 <= p <= 16])
        ...
        logger.info(f"Result save path: {args.output_path}")
        logger.info("=" * 80)
        ...
        logger.warning(f"\nNo high-memory samples filtered (PPL < {args.ppl_threshold})")
        logger.warning("Suggestions: 1. Lower PPL threshold (e.g., 12.0/14.0); 2. Check if model is compatible with Enron data")
