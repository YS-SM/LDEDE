...
class GPT2PhonePrivacyEvaluator:
    ...
    def evaluate_suppressed_exposure(self) -> None:
        ...
        self.suppressed_exposure = []
        for sample in tqdm(self.sample_details, desc="Calculating post-suppression Exposure", unit="sample"):
            try:
    ...
    def analyze_exposure_by_interval(self, intervals: List[Tuple[float, float]] = None) -> None:
        ...
        if len(self.baseline_exposure) != len(self.suppressed_exposure):
            raise ValueError("Pre/post suppression Exposure list lengths do not match")

        print(f"\n=== Exposure stratified analysis by interval (3-interval version) ===")
        ...
        if interval_stats:
            ...
            for i, stat in enumerate(best_intervals, 1):
                ...
                print(f"  No.{i}: {stat['interval_name']} ({min_b:.1f}-{max_b:.1f})")
    ...
    def calculate_ppl_comparison(self, model_path: str) -> None:
        ...
        if self.original_ppl != float('inf') and self.suppressed_ppl != float('inf'):
            ...
            print(
                f"Model performance evaluation: {'No significant loss' if ppl_increase < 30 else 'Slight loss' if ppl_increase < 50 else 'Severe loss'}")
        else:
...
def main():
    config = {
        ...
        "neuron_path": r"D:\expt\lrp_spin\main\phone\gpt2_phone_neurons_voted_top5000.npy",
    ...
    for hook in hooks:
        hook.remove()
    print("\nPhone privacy suppression verification completed!")
